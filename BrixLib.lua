...
function BrixLib:CreateWindow(options)
    ...
    -- Topbar for title + close/minimize
    ...
    -- Make window draggable via sidebar logo
    ...
    function WindowObj:CreateTab(name, icon)
        ...
        function TabObj:CreateSlider(options)
            ...
            local function UpdateSlider(input)
                ...
                local alpha = math.clamp((mouseX - sliderPos) / sliderSize, 0, 1)
        ...
        function TabObj:CreateDropdown(options)
            ...
            if flag and SavedConfig[flag] ~= nil then
                selected = SavedConfig[flag]
            end

            local isOpen = false

            -- Main dropdown frame (just the header bar)
            local DFrame = Instance.new("Frame")
            DFrame.Name = "DropdownFrame"
            ...
            local function UpdateSelectedText()
                if multi then
                    if #selected == 0 then
                        DSelectedLbl.Text = "None"
                    elseif #selected == 1 then
                        DSelectedLbl.Text = selected[1]
                    else
                        DSelectedLbl.Text = #selected .. " selected"
                    end
                else
            ...
            -- Dropdown List — parented to TabPage so it overlays properly
            ...
            for _, item in ipairs(items) do
                ...
                ItemBtn.MouseEnter:Connect(function()
                    if not isSelected(item) then
                        Tween(ItemBtn, {BackgroundColor3 = Color3.fromRGB(40, 40, 40)}, 0.1)
                    end
                end)
                ItemBtn.MouseLeave:Connect(function()
                    if not isSelected(item) then
                        Tween(ItemBtn, {BackgroundColor3 = Color3.fromRGB(32, 32, 32)}, 0.1)
                    end
                end)
            ...
            -- Position the list right below DFrame using AbsolutePosition
            local function UpdateListPosition()
                local absPos = DFrame.AbsolutePosition
                local absSize = DFrame.AbsoluteSize
                local pagePos = TabPage.AbsolutePosition
                DList.Position = UDim2.new(
                    0, absPos.X - pagePos.X,
                    0, absPos.Y - pagePos.Y + absSize.Y + 4
                )
                DList.Size = UDim2.new(0, absSize.X, 0, isOpen and maxHeight or 0)
            end
            ...
            DBtn.MouseButton1Click:Connect(function()
                ...
                if isOpen then
                    UpdateListPosition()
                    DList.Size = UDim2.new(0, DFrame.AbsoluteSize.X, 0, 0)
                    DList.Visible = true
                    Tween(DList, {Size = UDim2.new(0, DFrame.AbsoluteSize.X, 0, maxHeight)}, 0.2, Enum.EasingStyle.Back)
                    Tween(DArrow, {Rotation = 180}, 0.2)
                else
                    Tween(DList, {Size = UDim2.new(0, DFrame.AbsoluteSize.X, 0, 0)}, 0.15)
                    Tween(DArrow, {Rotation = 0}, 0.15)
        ...
        function TabObj:CreateKeybind(options)
            ...
            if flag and SavedConfig[flag] then
                pcall(function()
                    currentKey = Enum.KeyCode[SavedConfig[flag]]
                end)
            end
            ...
            UserInputService.InputBegan:Connect(function(input, gp)
                if listening and input.UserInputType == Enum.UserInputType.Keyboard then
                    ...
                    KBtn.TextColor3 = Theme.Accent
                    if flag then
                        SavedConfig[flag] = input.KeyCode.Name
                        SaveConfig(ConfigFolder, ConfigFile, SavedConfig)
                    end
                end
            end)
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    function WindowObj:CreateTab(name, icon)
        ...
        function TabObj:CreateDropdown(options)
            ...
            DArrow.Position = UDim2.new(1, -38, 0, 0)
            DArrow.BackgroundTransparency = 1
            DArrow.Text = "▼"
            DArrow.TextColor3 = Theme.Accent
            DArrow.TextSize = 11
            ...
            -- Selected display
            local DSelectedLbl = Instance.new("TextLabel")
            DSelectedLbl.Size = UDim2.new(1, -50, 1, 0)
            DSelectedLbl.Position = UDim2.new(0, 12, 0, 0)
            DSelectedLbl.BackgroundTransparency = 1
            DSelectedLbl.TextColor3 = Theme.TextDisabled
            DSelectedLbl.TextSize = 11
            DSelectedLbl.Font = Enum.Font.Gotham
            DSelectedLbl.TextXAlignment = Enum.TextXAlignment.Right
            DSelectedLbl.ZIndex = 5
            DSelectedLbl.Parent = DFrame
            ...
            DList.BackgroundColor3 = Theme.Secondary
            DList.BorderSizePixel = 0
            DList.ClipsDescendants = true
            DList.ZIndex = 10
            DList.Visible = false
